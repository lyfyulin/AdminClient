...
-- 昨日 平均出行距离                /od/last/avg_trip_time
...
-- 检索 评价出行次数                /search/od/avg_trip_freq
...
-- 各设备信息                       /device/each/info
<select id="getDeviceInfo" resultType="java.util.HashMap">
...
-- 车辆 路径 查询           /search/path/car_num
<select id="getPathByPlate" resultType="java.util.HashMap">
select to_char(cap_date, 'mm-dd hh24:mi') time_point,
    node_name,
    lat as olat,
...
-- 按要求检索 城区 常驻车辆             /search/changzhu/cnts?start_date=2020-04-16&end_date=2020-04-18&location=1&threshold=10
...
(
    ...
    (
        ...
        and b.设备位置 != #{location}
        group by a.car_num, a.cap_day having count(*) >= #{threshold}
    )
    group by car_num having count(*) >= #{4}
)
</select>
